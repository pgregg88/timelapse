...
def create_video(photo_path, srt_path, logger, timestamp, now, instance_name, fps, title, description, categoryId, playlistIds, tags, video_start_fade_time):
    ...
    try:
        ...
        with open(json_path, "w") as outfile:
            ...
            publish_mqtt_status("timelapse/status",f'JSON file created: {json_path}')
        
    except Exception as e:
            logger.error('Error creating JSON', exc_info=e)
            publish_mqtt_status("timelapse/error",'Error creating JSON', exc_info=e)
            publish_mqtt_status("timelapse/status",'Error creating JSON', exc_info=e)
    ...
    try:
        ...
        if os.path.exists(video_path):
            ...
            publish_mqtt_status("timelapse/status",f'Video created successfully: {video_path}')
            ...
            try:
                ...
                try:
                    ...
                    publish_mqtt_status("timelapse/error",'Error at %s', 'division', exc_info=e)
                if os.path.exists(archive_path):
                    shutil.rmtree(photo_path)
                    logger.info(f'Local copy of photos deleted: {photo_path}' )
                    publish_mqtt_status("timelapse/status",f'Local copy of photos deleted: {photo_path}' )
                else:
                    logger.error(f'Error: Could not delete local copy of photos: {photo_path}', exc_info=e)
                    publish_mqtt_status("timelapse/error",f'Error: Could not delete local copy of photos: {photo_path}', exc_info=e)
                    publish_mqtt_status("timelapse/status",f'Error: Could not delete local copy of photos: {photo_path}', exc_info=e)
                    
            except Exception as e:
                logger.error('Error creating photo archive', exc_info=e)
                publish_mqtt_status("timelapse/error",'Error creating photo archive', exc_info=e)
                publish_mqtt_status("timelapse/status",'Error creating photo archive', exc_info=e)
            return video_path, json_path
        else:
            logger.error('Error creating video')
            publish_mqtt_status("timelapse/error",'Error creating video')
            publish_mqtt_status("timelapse/status",'Error creating video')
            return None, None
        logger.info('Video complete.')
    except Exception as e:
        logger.error('Unknown error creating video', exc_info=e)
        publish_mqtt_status("timelapse/error",'Unknown error creating video', exc_info=e)
        publish_mqtt_status("timelapse/status",'Unknown error creating video', exc_info=e)
        return None, None
